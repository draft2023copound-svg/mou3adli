...
class ComboAnimation extends StatefulWidget {
  ...
  const ComboAnimation({
    super.key,
    required this.text,
    this.color = kWarningColor,
    this.fontSize = 32.0,
    this.onComplete,
  });

  factory ComboAnimation.score(
    int score, {
    int? combo,
    VoidCallback? onComplete,
  }) {
    return ComboAnimation(
      text: combo != null && combo > 1
          ? 'COMBO x$combo\n+${score.formatted}'
          : '+${score.formatted}',
      color: combo != null && combo > 2 ? kAccentColor : kSuccessColor,
      fontSize: combo != null && combo > 2 ? 40.0 : 28.0,
      onComplete: onComplete,
    );
  }

  factory ComboAnimation.perfectClear({VoidCallback? onComplete}) {
    return ComboAnimation(
      text: 'PERFECT CLEAR!',
      color: kWarningColor,
      fontSize: 48.0,
