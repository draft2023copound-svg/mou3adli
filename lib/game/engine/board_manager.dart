...
class BoardManager {
  ...
  PlacementResult placePiece(Piece piece, int row, int col) {
    ...
    final rows = board.getCompleteRows();
    final cols = board.getCompleteColumns();

    int cellsCleared = 0;
    bool perfectClear = false;

    if (rows.isNotEmpty || cols.isNotEmpty) {
      cellsCleared = board.clearLinesAndColumns(
        rowsToClear: rows,
        colsToClear: cols,
      );
    ...
    return PlacementResult(
      success: true,
      clearedRows: List.unmodifiable(rows),
      clearedCols: List.unmodifiable(cols),
  ...
  List<(int row, int col)> getValidPositions(Piece piece) =>
      board.getValidPositionsForPiece(piece);
